...
def main(name, argv):
        ...
        params = utils.read_params(config_file=argv[0])
        Structs = params['Structures'].split()
        Chains = params['Chains'].split()
        Heads = params['Heads'].split()
        Anchors = [int(i) - 1 for i in params['Anchor atoms'].split()]
        Linkers = params['Protac']
        Full = params['Full'] == 'True'

        log.write('INFO: Using these params:\n')
        for k, v in params.items():
                log.write(f'INFO:   {k}: {v}\n')

        # Get a handle to the cluster specified in config file (or default, which is PBS).
